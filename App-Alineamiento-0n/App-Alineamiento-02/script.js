...
function irAlFinal() {
    ...
    if (paso === (n - 1) * (m - 1) + 1) {
        ...
        if(algoritmo === "sw") {
            caminos = tracebackSW(matriz);
            if (caminos && caminos.length > 0 && caminos[0].pasos) {
                limite = paso + caminos[0].pasos.length;
            } else {
                siguiente(algoritmo, (n - 1) * (m - 1));
                return;
            }
        }
        if(algoritmo === "nw") {
            caminos = tracebackNW(matriz);
            if (caminos && caminos.pasos) {
                limite = paso + caminos.pasos.length;
            }
        }
        paso++;
    }
    if (algoritmo === "sw" && caminos && caminos.length > 0 && caminos[0].pasos) {
        while (posicionActualCamino < caminos[0].pasos.length) {
            avanzar(algoritmo,
                caminos[0].coordenadas[posicionActualCamino][0],
                caminos[0].coordenadas[posicionActualCamino][1],
                caminos[0].pasos[posicionActualCamino]);
            posicionActualCamino++;
            paso++;
        }
    }

    if (algoritmo === "nw" && caminos && caminos.pasos) {
        while (posicionActualCamino < caminos.pasos.length) {
            avanzar(algoritmo,
                caminos.coordenadas[posicionActualCamino][0],
                caminos.coordenadas[posicionActualCamino][1],
                caminos.pasos[posicionActualCamino]);
            posicionActualCamino++;
            paso++;
        }
        final = 1;
    }
    if(final === 1 && algoritmo === "nw"){
        avanzar("nw", 0, 0, "final");
        paso++;
    }
}

document.getElementById("irAlFinal").addEventListener("click", function (event) {
    event.preventDefault();
    irAlFinal();
});