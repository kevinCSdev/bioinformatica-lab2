function anterior(algoritmo, p) {
    ...
    let s1 = (coordenadas2[0] > 0) ? seq1[coordenadas2[0] - 1] : '';
    let s2 = (coordenadas2[1] > 0) ? seq2[coordenadas2[1] - 1] : '';
    ...
    const matrixColumn = root.append("div").attr("class", "matrix-column")

    const svg = matrixColumn
        .append("svg")
        .style("max-width", "100%")
        .attr(
            "viewBox",
    ...
    const infoContainer = root.append("div").attr("class", "info-column")
    
    let equation = createEquation(puntajesMaximos, algoritmo);
    infoContainer.append("p").html(`\\(${equation}\\)`);
    
    const diagonalAnterior = obtenerPuntaje(coordenadas2[0] - 1, coordenadas2[1] - 1);
    const superiorAnterior = obtenerPuntaje(coordenadas2[0] - 1, coordenadas2[1]);
    const lateralAnterior = obtenerPuntaje(coordenadas2[0], coordenadas2[1] - 1);
    
    const matrizHTML = crearMatrizCalculo({
        diagonal: diagonalAnterior,
        superior: superiorAnterior,
        lateral: lateralAnterior,
        diagonalScore: score, 
        gapScore: gap,
        diagonalResult: diagonal,
        superiorResult: superior,
        lateralResult: lateral,
        maxResult: maximoPuntaje, 
        char1: s1 || '?',
        char2: s2 || '?',
        isMatch: s1 === s2,
        algoritmo: algoritmo
    ...
    calcContainer.appendChild(title);
    calcContainer.appendChild(matrizHTML);
    infoContainer.node().appendChild(calcContainer);
