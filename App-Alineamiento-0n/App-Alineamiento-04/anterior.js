function anterior(algoritmo, p) {
    ...
    // Si es ED, reconstruir edges hasta el paso anterior para poder mostrar correctamente
    if (algoritmo === 'ed' && typeof window.getEDEdgesAtStep === 'function') {
        edges = window.getEDEdgesAtStep(paso - 1);
    }
    ...
    else if (algoritmo === "ed") puntajes = [diagonal, superior, lateral]
    
    // Para ED usamos mínimo, para NW/SW usamos máximo
    let ganadorPuntaje = (algoritmo === 'ed') ? Math.min(...puntajes) : Math.max(...puntajes)

    let puntajesMaximos = []
    if (diagonal === ganadorPuntaje) puntajesMaximos.push('diagonal')
    if (superior === ganadorPuntaje) puntajesMaximos.push('superior')
    if (lateral === ganadorPuntaje) puntajesMaximos.push('lateral')
    if (algoritmo === "sw" && ganadorPuntaje === 0) puntajesMaximos.push('zero')
    ...
    if (algoritmo === "nw") {
        ...
        root.append("h1").text("Distancia de edición: Wagner–Fischer");
    }
