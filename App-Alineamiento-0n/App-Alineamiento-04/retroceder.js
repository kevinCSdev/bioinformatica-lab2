function retroceder(algoritmo, i, j, p) {
    ...
    switch (algoritmo) {
        case "nw":
            alineamientos = alineamientoNW();
            break;
        case "sw":
            alineamientos = alineamientoSW();
            break;
        case "ed":
            alineamientos = alineamientoED();
            break;
        default:
            alineamientos = { seq1: "", seq2: "", puntaje: 0 };
    }
...
function borrarCamino(x, y,) {
    ...
    if (edge) {
        edge.camino = 0
        // Limpiar flags de flecha para que no queden marcadores al retroceder
        edge.diagonalF = 0
        edge.superiorF = 0
        edge.lateralF = 0
    }
...
function alineamientoSW() {
    const seq1 = $('#seq_1').val()
    const seq2 = $('#seq_2').val()
    // `caminos` puede ser un arreglo (SW) o un objeto (NW/ED wrapper). Manejar ambos.
    let caminoObj = Array.isArray(caminos) ? caminos[0] : caminos;
    if (!caminoObj) { return { seq1: '', seq2: '', puntaje: 0 }; }
    let coordenadas = caminoObj.coordenadas || [];
    let pasos = caminoObj.pasos || [];
